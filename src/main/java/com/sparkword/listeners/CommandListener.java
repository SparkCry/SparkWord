        ...
        if (muteInfo.blocks(MuteScope.GLOBAL)) {
            ...
            if (parts.length > 0) {
                ...
                if (BLOCKED_CMDS.contains(cmd)) {
                    ...
                    plugin.getEnvironment().getMessageManager().sendMessage(event.getPlayer(), "player-muted",
                        Map.of("staff", muteInfo.staff(), "time", timeLeft, "reason", "PERMUTE (Commands blocked)"));
                    return;
                }
            }
        }

        if (event.getPlayer().hasPermission("sparkword.bypass.writecommands")) return;
