...
public class SignListener implements Listener {
    ...
    public void onSignChange(SignChangeEvent event) {
        ...
        if (muteInfo.blocks(MuteScope.GLOBAL)) {
            ...
            plugin.getEnvironment().getMessageManager().sendMessage(player, "prevention.blocked-permute.sign");
            return;
        }
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (filterResult.blocked()) {
                event.setCancelled(true);
                plugin.getEnvironment().getMessageManager().sendMessage(player, "notify-blocked", Map.of("reason", filterResult.reason()));
            ...
            if (!filterResult.detectedWords().isEmpty()) {
                ...
                if (processed.equals(lineComponent)) {
                    event.setCancelled(true);
                    plugin.getEnvironment().getMessageManager().sendMessage(player, "notify-blocked", Map.of("reason", "Evasion"));
