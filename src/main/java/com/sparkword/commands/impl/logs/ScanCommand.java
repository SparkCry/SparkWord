...
public class ScanCommand implements SubCommand {
    ...
    public ScanCommand(Environment env) { this.env = env; }
    ...
    public boolean execute(CommandSender sender, String[] args) {
        ...
        if (args.length < 1) {
            env.getMessageManager().sendMessage(sender, "usage-scan");
            return true;
        }
        ...
        CompletableFuture.supplyAsync(() -> {
            ...
            return env.getStorage().getPlayerReportAsync(pid, fPage);
        }).thenAccept(reportLines -> {
            Bukkit.getScheduler().runTask(env.getPlugin(), () -> {
                sender.sendMessage(MiniMessage.miniMessage().deserialize(
                    "<dark_gray>--- <#09bbf5>Scan: " + targetName + " (Pg " + fPage + ")</#09bbf5> <dark_gray>---"
                ));

                if (reportLines.isEmpty()) {
                    env.getMessageManager().sendMessage(sender, "scan-empty");
                } else {
                    for (String str : reportLines) sender.sendMessage(str);
                }
            });
        });
